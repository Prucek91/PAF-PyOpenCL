...
def action(N_, iter=-1):
    ...
    f = open('adjust_score3.cl', 'r', encoding='utf-8')
    ...
    upload_data_start_time = time.time()#time.time()
    ...
    dev_c_matrix = cl.Buffer(ctx, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=c)
    dev_F_matrix = cl.Buffer(ctx, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=F)
    ...
    #cl.enqueue_copy(queue, final, dev_final_matrix).wait()
...
if __name__ == '__main__':
    ...
    if TYPE == 1:
        iters = 5
        Ns = list(np.arange(200, 2301, 100))
        Ns.reverse()
